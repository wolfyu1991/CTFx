 ...
 */
export function pduDecoder(pdu) {
    let i;
    const result = [];

    const octets = splitter(pdu);

    if (!octets) {
        return "Invalid PDU String!";
    }

    const tokens = tokenizer(octets);
...
function tokenizer(octets) {
    ...
    // smsc part
    const smscLength = parseInt(octets[0], 16);

    if (smscLength) {
        const sliceSmsc = octets.slice(2, smscLength + 1);
        const sliceSmscToA = octets[1];
        tokenList.push(function () {
            return "(hideable)SMSC number\t" + tokens.Number(sliceSmsc, undefined, tokens.ToA(sliceSmscToA));
        });
        tokenList.push(function () {
            return "(hideable)SMSC number info\t" + tokens.ToA(sliceSmscToA).info;
        });
    }

    // Sender/Receiver part
    pos = smscLength + 1;
    const pduType = tokens.ToM(octets[pos]);
    tokenList.push(function () {
        return "(hideable)PDU Type\t" + pduType.info;
    });
    ...
    if (pduType.type === "deliver") {
        ...
        if (numberLength) {
            ...
            tokenList.push(function () {
                return "(hideable)Number\t" + tokens.Number(sliceNumber, numberLength, tokens.ToA(sliceNumberToA));
            });
            tokenList.push(function () {
                return "(hideable)Number info\t" + tokens.ToA(sliceNumberToA).info;
            });
        ...
        TP_PID = octets[pos];
        tokenList.push(function () {
            return "(hideable)Protocol Identifier\t" + tokens.PID(TP_PID);
        });

        pos++;
        TP_DCS = tokens.DCS(octets[pos]);
        tokenList.push(function () {
            return "(hideable)Data Coding Scheme\t" + TP_DCS.info;
        });

        pos++;
        const sliceTimeStamp = octets.slice(pos, pos + 7);
        tokenList.push(function () {
            return "(hideable)Service Centre Time Stamp\t" + tokens.SCTS(sliceTimeStamp);
        });
        ...
        const MR = octets[pos];
        tokenList.push(function () {
            return "(hideable)TP Message Reference\t" + tokens.MR(MR);
        });
        ...
        if (pduType.TP_VPF) {
            ...
            if (pduType.TP_VPF === "relative") {
                sliceVP = octets[pos];
                tokenList.push(function () {
                    return "(hideable)Validity Period\t" + tokens.VPrelative(sliceVP);
                });
            } else if (pduType.TP_VPF.match(/^(absolute|relative)$/)) {
                sliceVP = octets.slice(pos, pos + 7);
                tokenList.push(function () {
                    return "(hideable)Validity Period\tuntil " + tokens.SCTS(sliceVP);
                });
                pos += 6;
            }
        }
    ...
    pos++;
    const TP_UDL = tokens.UDL(octets[pos], TP_DCS.alphabet);
    tokenList.push(function () {
        return "User Data Length\t" + TP_UDL.info;
    });
    ...
    if (pduType.TP_UDHI) {
        ...
        tokenList.push(function () {
            return "User Data Header Length\t" + TP_UDHL.info;
        });

        pos++;
        TP_UDH = tokens.UDH(octets.slice(pos, pos + TP_UDHL.length));
        tokenList.push(function () {
            return "User Data Header\t" + TP_UDH.info;
        });
        pos += TP_UDHL.length - 1;
    }

    pos++;
    const expectedMsgEnd = pos + TP_UDL.octets - (TP_UDHL.length ? TP_UDHL.length + 1 : 0);
    const sliceMessage = octets.slice(pos, expectedMsgEnd);

    if (TP_UDH.wap) {
        const wapMessage = wapDecoder(sliceMessage);
        tokenList.push(function () {
            return "User Data\tWireless Session Protocol (WSP) / WBXML " + wapMessage;
        });
    } else {
        tokenList.push(function () {
            return "User Data\t" + tokens.UD(sliceMessage, TP_DCS.alphabet, TP_UDHL.padding, TP_UDH.formatting);
        });
        ...
        if (expectedMsgEnd < octets.length) {
            ...
            const sliceMessageAll = octets.slice(pos, octets.length);
            tokenList.push(function () {
                return "User Data /w additional stuff\t" + tokens.UD(sliceMessageAll, TP_DCS.alphabet, TP_UDHL.padding, TP_UDH.formatting);
            });
...
const tokens = {
    ...
    Number: function (octets, length, addressType) {
        ...
        if (addressType && addressType.ToN === 0x50) {
            ...
            if (number.match(/\D$/) || (length && number.length > length)) {
                const paddingEx = /(.)$/;
                const result = paddingEx.exec(number);
     ...
     */
    ToA: function (octet) {
        const type = parseInt(octet, 16);

        const ToN = type & 0x70; // Type of number Bits
        const NPI = type & 0xF;	// Numbering Plan Identification
     ...
     */
    ToM: function (octet) {
        const o = parseInt(octet, 16);
        const TpMtiMask = 0x1; // 0x3;
        let text = "";
        const flags = [];
        let deliver = false;
        let submit = false;
        ...
        if (submit) {
            ...
            const TpVpfMask = o & 0x18;
            const vpfText = "TP-VPF (Validity Period Format): ";
            ...
            } else if (TpVpfMask === 0x10) {
                TP_VPF = "relative";
                flags.push(vpfText + "relative format");
            } else if (TpVpfMask === 0x18) {
                TP_VPF = "absolute";
    ...
    UDHL: function (octet, alphabet) {
        const length = parseInt(octet, 16);
        let padding = 0;

        if (alphabet === "default") {
            const udhBitLength = (length + 1) * 8;
            const nextSeptetStart = Math.ceil(udhBitLength / 7) * 7;
    ...
    UDH: function (octets) {
        ...
        const IEs = [],		// all Information Elements
            info = [],
            formatting = [],
            ems = [];
        ...
        while (octets.length) {
            const o = parseInt(octets.shift(), 16);
        ...
        for (i = 0; i < IEs.length; ++i) {
            if (IEs[i].IEI === 5) {
                ...
                formatting.push(function (text, original, i) {
                    ...
                    const getPart = new RegExp(original);
...
function decode7Bit(octets, padding) {
    let thisAndNext, thisChar, character,
        nextChar = "",
        text = "";
...
function getChar(octets, nextChar) {
    ...
    const octet = padwZeros(parseInt(octets.shift(), 16).toString(2));
    const bitsFromNextChar = nextChar.length + 1;
    const thisChar = octet.substr(bitsFromNextChar) + nextChar;
...
function wapDecoder(octets) {
    ...
    pos++;
    const headerLength = parseInt(octets[pos], 16);
    pos++;
