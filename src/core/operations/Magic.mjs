...
class Magic extends Operation {
    ...
    constructor() {
        ...
        this.module = "Default";
        this.description = "Magic操作尝试检测输入数据的多种属性来建议对应的操作。<br><br><b>设定</b><br><u>深度：</u>如果某个操作和数据相符，此操作会被运行一次，运行结果会被再次分析。此参数设置递归的最大次数。<br><br><u>增强模式：</u>当此模式开启时，多种操作如XOR、位运算和字符编码会进行暴力破解来检测可能的数据处理方式。为提高性能，仅有前100个字节会被暴力破解。<br><br><u>扩展语言支持：</u>在每个阶段，数据的相对字节频率数据会和各种现实语言的平均频率进行对照。默认情况下仅检测约40种互联网常见语言。扩展语言功能将语言数据提高到284种，这使得数据可以匹配更多语言种类，如果它们的频率数据相近的话。<br><br>支持输入一个正则表达式来过滤输出结果。";
